...
|---|---|
| host | ホスト名 |
| reachable | `yes` = HTTPで応答あり／`no` = 名前解決はできたが応答なし |
...
名前解決はできたが HTTP で応答がなかったホストも `assets.csv` に記録します
（`reachable` 列が `no`）。

DNSにAレコードがある以上、そのホストは実在します。応答がない理由は次のいずれかです。

- HTTP以外で稼働している（VPN、SSH、メールなど）
- スキャン元のIPを遮断している
- 一時的に停止している

**「応答なし」は「安全」でも「存在しない」でもありません。**
特にVPN機器などは、外部から中身を確認できないため、
版数を別途台帳で管理する必要があります。

前回応答したホストが今回応答しなかった場合、「今回は検出されなかった」として
報告しますが、停止したとは限りません。判定は応答の有無ではなく
ホスト名で行うため、応答状態が変わっただけでは差分になりません。
